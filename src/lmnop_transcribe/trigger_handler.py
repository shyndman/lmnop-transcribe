...
async def wait_for_start_trigger(device: evdev.InputDevice, config: Config):
  ...
  async for event in device.async_read_loop():
    logger.debug(f"Received event: type={event.type}, code={event.code}, value={event.value}")
    if event.type == evdev.ecodes.EV_KEY:
      logger.debug(f"Key event: type={event.type}, code={event.code}, value={event.value}")
